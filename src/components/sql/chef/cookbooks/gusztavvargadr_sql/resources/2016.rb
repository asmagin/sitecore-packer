...
action :install do
  directory_path = "#{Chef::Config[:file_cache_path]}/gusztavvargadr_sql/2016_#{edition}"
  ...
  installer_iso_source = node['gusztavvargadr_sql']["2016_#{edition}"]['installer_iso_url']
  ...
  gusztavvargadr_windows_powershell_script_elevated "Install SQL Server 2016 #{edition}" do
    code <<-EOH
