...
action :install do
  directory_path = "#{Chef::Config[:file_cache_path]}/scp_sql/2014_#{edition}"
  ...
  installer_file_name = node['scp_sql']["2014_#{edition}"]['installer_file_name']
  installer_file_path = "#{directory_path}/#{installer_file_name}"
  installer_file_source = node['scp_sql']["2014_#{edition}"]['installer_file_url']
  ...
  if installer_file_extension == '.iso'
    ...
    scp_windows_iso installer_file_path do
      iso_drive_letter 'I'
      action :mount
    end
  else
    extracted_directory_path = "#{directory_path}/extracted"
    scp_windows_powershell_script_elevated "Extract SQL Server 2014 #{edition}" do
      code <<-EOH
  ...
  scp_windows_powershell_script_elevated "Install SQL Server 2014 #{edition}" do
    code <<-EOH
