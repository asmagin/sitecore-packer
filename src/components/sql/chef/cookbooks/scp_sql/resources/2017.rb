...
action :install do
  directory_path = "#{Chef::Config[:file_cache_path]}/scp_sql/2017_#{edition}"
  ...
  installer_iso_source = node['scp_sql']["2017_#{edition}"]['installer_iso_url']
  ...
  scp_windows_powershell_script_elevated "Install SQL Server 2017 #{edition}" do
    code <<-EOH
