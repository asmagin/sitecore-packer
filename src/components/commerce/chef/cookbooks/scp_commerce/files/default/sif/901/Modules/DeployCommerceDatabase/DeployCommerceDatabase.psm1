...
function Invoke-DeployCommerceDatabaseTask {
    ...
    param(
        ...
        [string]$SqlCommerceServicesDbServer,
		[Parameter(Mandatory=$true)]
        [string]$SqlCommerceServicesDbName,
        [Parameter(Mandatory=$true)]
        [string]$SqlCommerceServicesGlobalDbName,
		[Parameter(Mandatory=$true)]
        [string]$PackageCommerceEngineDacPacPath,	        		
        [Parameter(Mandatory=$true)]
	...
	Add-SQLPSSnapin;
	DropSQLDatabase $SqlCommerceServicesDbName
	DropSQLDatabase $SqlCommerceServicesGlobalDbName

	Write-Host "Creating CommerceServices databases...";	
	$connectionString = "Server=" + $SqlCommerceServicesDbServer + ";Trusted_Connection=Yes;"	
	$userName = "$($UserAccount.domain)\$($UserAccount.username)"
	
	#deploy using the dacpac
	try {
		deploydacpac $PackageCommerceEngineDacPacPath $connectionString $SqlCommerceServicesGlobalDbName
		deploydacpac $PackageCommerceEngineDacPacPath $connectionString $SqlCommerceServicesDbName
		write-host "adding roles to commerceservices databases...";
		AddSqlUserToRole $SqlCommerceServicesDbServer $SqlCommerceServicesGlobalDbName $userName "db_owner"
		AddSqlUserToRole $SqlCommerceServicesDbServer $SqlCommerceServicesDbName $userName "db_owner"
	}
	catch {
...
function Invoke-AddCommerceUserToCoreDatabaseTask {
    ...
    param(
        [Parameter(Mandatory=$true)]
        [string]$SqlSitecoreDbServer,
		[Parameter(Mandatory=$true)]
        [string]$SqlSitecoreCoreDbName,	
        [Parameter(Mandatory=$true)]
        [psobject[]]$UserAccount = @()
    )
    ...
    try {
	    AddSqlUserToRole $SqlSitecoreDbServer $SqlSitecoreCoreDbName $userName "db_owner"
	}
	catch {
        Write-Error $_ -ErrorAction Continue
	}	
