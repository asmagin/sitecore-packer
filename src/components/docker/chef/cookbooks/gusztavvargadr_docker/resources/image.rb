...
action :pull do
  image_script_name = "image-#{Time.now.to_i}"
  image_script_code = <<-EOH
    timeout /t 120 > nul
    docker pull #{image_name}
  EOH
  image_script_directory_path = "#{Chef::Config[:file_cache_path]}/gusztavvargadr_docker"
  directory image_script_directory_path do
    recursive true
    action :create
  end

  image_script_file_name = "#{image_script_name}.cmd"
