...
try {
	...
	Write-Error "keytool.exe not on path. Enter path to keytool (found in JRE bin folder)"
}
...
$args = @(
	...
	"$KeystoreFile"
	"-ext"
	"SAN=DNS:$SolrDomain,IP:127.0.0.1"
	"-dname"
	"""CN=$SolrDomain, OU=Organizational Unit, O=Organization, L=Location, ST=State, C=Country"""
)

$p = Start-Process $keytool -ArgumentList $args -wait -NoNewWindow -PassThru -RedirectStandardOutput 'keytool-output.txt'
$p.ExitCode
...
$p = Start-Process $keytool -ArgumentList $args2 -wait -NoNewWindow -PassThru -RedirectStandardOutput 'keytool-output.txt'
$p.ExitCode
